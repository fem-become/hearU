 ...
 */
 (function(){
	...
	richSong = function(songs,index,res,callback){
		...
		collectId;
		song.hasFavor = song.hasFavor || false;
	...
	gatherSongs = function(songs,collectIds,index,res,callback){
		var collectId = collectIds[index];
		collect.findOne
	};
	...
	exports.search = function(key,userId,callback,res){
		...
		user.findOne({_id:ObjectID(userId)},function(err,item){
			if(item){
				...
				collect.find({_id:{$in : turnToObjectId(myCollects)}}).toArray(function(err,items){
					for(var i=0,l=items.length;i<l;i++){
						mySongs = mySongs.concat(items[i].songs || []);
					}
                    console.info('----------------');
					...
					song.find(condition).toArray(function(err,items){
			            if(items.length){
			            	for(var i=0,l=items.length;i<l;i++){
			            		songId = items[i]._id.toString();
                                console.info(songId);
                                console.info(mySongs.indexOf(songId) > -1);
			            		items[i].hasFavor = (mySongs.indexOf(songId) === -1)?false:true;
                                console.info(items[i].hasFavor);
			            	}
                            console.info(items);
