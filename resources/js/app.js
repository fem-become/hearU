...
//                HearU.createItem();
	...
	var HearU = {
        ...
        openSelect: function(songId, oldCollectId, cb, donecb, notFavor) {
            if (this.hasSelect) {
                return;
            }
            //从缓存中读取歌单数据
            ...
            SpinningWheel.setDoneAction(function(){
                ...
                if (!notFavor) {
                    url = "/collect/moveSong";
                    params = {songId: songId, oldCollectId: oldCollectId, newCollectId: newCollectId};
                } else {
                    url = "/collect/addSong";
                    params = {songId: songId, collectId: newCollectId};
                }

                HearU.requestAPI(url, params, function(d) {
                    //do nothing
                });
                HearU.closeSelect();
...
//                            }
                        ...
                        if(direction == 'left' || direction=='right'){
                            ...
                            if(distance > 0){
                                if(Math.abs(distance) > WIN_WIDTH*0.2) {
...
//                                this.current_view.swipeRight && this.current_view.swipeRight(ev);
                        ...
                        $('.check',$parent).removeClass("animated animate-repeat pulse");
                        $('.cross',$parent).removeClass("animated animate-repeat shake");
                    }
                        break;
                    case 'tap': {
                        var $parent;
        ...
        search: function(keyword) {
            var self = this;
            HearU.requestAPI('/song/search', {key: keyword, userId: window.sessionId}, function(dt) {
                var data = HearU.searchResult = dt;

                var html = [];
                $.each(data, function(idx, item) {
                    html.push('<li class="song item" data-index="'+idx+'">');
                    html.push('<div class="slider"><i class="icon-play"></i>'+item.name + " - " + item.artist+'</div>');
        ...
        closeSearch: function() {
            ...
            $('.search-box input').blur().attr('defaultValue', '');
